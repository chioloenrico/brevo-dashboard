...
- **THEN** the function SHALL make a GET request to `https://api.brevo.com/v3/emailCampaigns?statistics=globalStats` with header `api-key` containing the value from `process.env.BREVO_API_KEY`
